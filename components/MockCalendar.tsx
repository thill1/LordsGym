...
// Generate Mock Data for Jan 2025
const generateJanuarySchedule = () => {
  ...
  for (let day = 1; day <= 31; day++) {
    ...
    // Jan 1 is Wed (3). 
    // formula: (day + 2) % 7 where 1st=3(Wed)
    // 1=Wed(3), 2=Thu(4), 3=Fri(5), 4=Sat(6), 5=Sun(0), 6=Mon(1)
    const dayOfWeek = (day + 2) % 7; 
    
    const dailyEvents: CalendarEvent[] = [];
    const datePrefix = `jan-${day}`;
    ...
    // Special case: Jan 1 (New Years)
    if (day === 1) {
       // Clear events and add special
       dailyEvents.length = 0;
       dailyEvents.push({ id: 'ny-1', title: 'New Year Open Gym', time: '10:00 AM', instructor: 'Staff', type: 'community' });
    }
...
const SCHEDULE = generateJanuarySchedule();

const MockCalendar: React.FC = () => {
  const [selectedDay, setSelectedDay] = useState<number | null>(null);

  // Jan 1, 2025 is Wednesday. 
  // Padded days at start: Sun(0), Mon(1), Tue(2) -> 3 empty slots
    ...
    <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-xl overflow-hidden border border-neutral-200 dark:border-neutral-700">
      ...
      <div className="grid grid-cols-7 auto-rows-fr bg-neutral-200 dark:bg-neutral-700 gap-px border-b border-neutral-200 dark:border-neutral-700">
        ...
        {daysInMonth.map(day => {
          ...
          const isToday = day === 6; // Mocking "Today" as Jan 6th for demo visual
      ...
      {/* Selected Day Details (Mobile Friendly View) */}
