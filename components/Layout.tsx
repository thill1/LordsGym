...
const Layout: React.FC<LayoutProps> = ({ children, currentPath, onNavigate }) => {
  ...
  // Transparent over hero: use white logo. Once scrolled: use dark logo.
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <div className="cursor-pointer relative z-50" onClick={() => handleNavClick('/')}>
            <Logo variant="nav" tone={isTransparent ? "light" : "dark"} />
          </div>
          ...
          <div className="lg:hidden flex items-center gap-4 relative z-50">
            ...
            >
              <div className="w-6 h-6 flex flex-col justify-center gap-1.5">
                <span className={`block h-0.5 w-6 bg-current transform transition-transform duration-300 ${isMobileMenuOpen ? 'rotate-45 translate-y-2' : ''}`} />
                <span className={`block h-0.5 w-6 bg-current transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-0' : 'opacity-100'}`} />
                <span className={`block h-0.5 w-6 bg-current transform transition-transform duration-300 ${isMobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
              </div>
            </button>
          </div>
