  ...
  return (
    <div className="min-h-screen flex flex-col font-sans relative">
      ...
      {settings.announcementBar.enabled && (
        <div className="bg-brand-red text-white text-xs font-bold uppercase tracking-widest py-2 text-center cursor-pointer hover:bg-brand-redHover transition-colors relative z-[60]" onClick={() => settings.announcementBar.link && handleNavClick(settings.announcementBar.link)}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {NAV_ITEMS.map((item) => (
              ...
              >
                {item.label}
                <span className={`absolute bottom-0 left-0 h-0.5 bg-brand-red transition-all duration-300 ease-out ${currentPath === item.path ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
              </button>
            ))}
          </nav>
