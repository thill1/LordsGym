...
const Layout: React.FC<LayoutProps> = ({ children, currentPath, onNavigate }) => {
  ...
  // Logo tone: white logo when header is transparent; dark logo when header background is white
  const navLogoTone = isTransparent ? "light" : "dark";
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans relative">
      ...
      {settings.announcementBar.enabled && (
        <div
          className="bg-brand-charcoal text-white text-xs font-bold uppercase tracking-widest py-2 text-center cursor-pointer hover:bg-black transition-colors relative z-[60]"
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <div className="cursor-pointer relative z-50" onClick={() => handleNavClick("/")}>
            <Logo variant="nav" tone={navLogoTone} />
          </div>
            ...
            {NAV_ITEMS.map((item) => (
              <button
                ...
                className={`text-sm uppercase tracking-widest font-bold hover:text-brand-red transition-colors relative group py-2 ${
                  currentPath === item.path ? "text-brand-red" : "text-current"
                }`}
              >
                {item.label}
                <span
          ...
          <div className="hidden lg:flex items-center space-x-5">
            ...
            <Button size="sm" variant="brand" onClick={() => handleNavClick("/membership")}>
              Join Now
            </Button>
          </div>
      ...
      >
        <div className="flex flex-col space-y-6 text-center w-full px-8 max-w-sm">
          ...
          <div className={`pt-8 space-y-4 w-full transition-all duration-500 delay-300 transform ${isMobileMenuOpen ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"}`}>
            <Button fullWidth variant="brand" size="lg" onClick={() => handleNavClick("/membership")}>
              Join Now
            </Button>
            <Button
              variant="outline"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
            <div className="space-y-6">
              <Logo variant="footer" tone="light" className="origin-left" />
