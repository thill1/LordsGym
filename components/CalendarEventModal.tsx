...
const CalendarEventModal: React.FC<CalendarEventModalProps> = ({
  ...
  const remaining = event.capacity ? event.capacity - (event.booked_count ?? 0) : null;
  const pct = event.capacity ? Math.round(((event.booked_count ?? 0) / event.capacity) * 100) : 0;
  const isFull = remaining !== null && remaining <= 0;

  const recurringLabel = (() => {
    const pat = event.recurring_pattern;
    if (!pat) return null;
  ...
  const colorClass = getClassTypeColor(event.class_type);
      ...
      >
        ...
        <div className={`px-5 py-4 sm:rounded-t-xl ${colorClass} border-b border-neutral-200 dark:border-neutral-700`}>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <span className="text-xs font-bold uppercase opacity-80">{formatClassType(event.class_type)}</span>
              <h2 className="text-xl font-bold mt-0.5 truncate">{event.title}</h2>
            </div>
            <button
              onClick={onClose}
              aria-label="Close"
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-bold dark:text-white">{formatDate(event.start_time)}</p>
              <p className="text-sm text-brand-red font-bold">
                {allDay ? 'All Day' : `${formatTime(event.start_time)} - ${formatTime(event.end_time)}`}
              </p>
            </div>
          </div>
          ...
          {recurringLabel && (
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {event.recurring_pattern?.end_date && (
                  <p className="text-xs text-neutral-500 dark:text-neutral-400">Until {new Date(event.recurring_pattern.end_date).toLocaleDateString()}</p>
                )}
              </div>
            </div>
          )}
          ...
          {event.description && (
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-neutral-700 dark:text-neutral-300 leading-relaxed">{event.description}</p>
            </div>
          )}
                ...
                <div className="flex items-center justify-between mb-1">
                  <p className="text-sm font-medium dark:text-white">
                    {remaining != null && remaining > 0
                      ? `${remaining} of ${event.capacity} spots remaining`
                      : isFull ? 'Class is full' : `${event.capacity} spots`}
                  </p>
                  <span className="text-xs text-neutral-400">{event.booked_count ?? 0}/{event.capacity}</span>
                </div>
          ...
          {onBook && (
            <div className="pt-4 border-t border-neutral-200 dark:border-neutral-700">
              {showBookingForm ? (
                <CalendarBookingForm
                  eventId={event.id}
                  onSuccess={() => { onBook(event.id); onClose(); }}
