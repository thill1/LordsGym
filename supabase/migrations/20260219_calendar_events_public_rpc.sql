-- RPC to fetch calendar events with recurring patterns for public (anon) visitors.
-- Bypasses RLS on calendar_recurring_patterns so recurring events show on mobile Safari.
-- The public calendar MUST show recurring events; RLS was blocking the join for anon users.
...
-- Allow anon and authenticated to call
